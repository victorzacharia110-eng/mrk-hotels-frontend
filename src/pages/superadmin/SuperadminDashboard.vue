    ...
    <template v-else-if="data">
      <div class="dash-header">
        ...
        <span class="role-badge"
          ><i class="fas fa-shield-halved"></i> {{ $t('superadmin.title') }}</span
        >
      </div>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div>
            <span class="stat-value">{{ data.tenants.total }}</span
            ><span class="stat-label">{{ $t('superadmin.totalTenants') }}</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon active"><i class="fas fa-check-circle"></i></div>
          <div>
            <span class="stat-value">{{ data.tenants.active }}</span
            ><span class="stat-label">{{ $t('superadmin.statusActive') }}</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon pending"><i class="fas fa-clock"></i></div>
          <div>
            <span class="stat-value">{{ data.tenants.pending_approvals }}</span
            ><span class="stat-label">{{ $t('superadmin.pendingApprovals') }}</span>
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-icon suspended"><i class="fas fa-ban"></i></div>
          <div>
            <span class="stat-value">{{ data.tenants.suspended }}</span
            ><span class="stat-label">{{ $t('superadmin.statusSuspended') }}</span>
          </div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
          ...
          <div>
            <span class="stat-value">{{ data.subscriptions.trials_expiring_7_days }}</span
            ><span class="stat-label">{{ $t('superadmin.trialsExpiring') }}</span>
          </div>
        </div>
      </div>
      ...
      <template v-if="data.saas">
        <h2 class="saas-title">
          <i class="fas fa-chart-line"></i> {{ $t('superadmin.saasBusiness') }}
        </h2>
        <div class="stats-grid">
          <div class="stat-card">
            ...
            <div>
              <span class="stat-value"
                >{{ data.saas.currency }} {{ data.saas.mrr.toLocaleString() }}</span
          ...
          <div class="stat-card">
            ...
            <div>
              <span class="stat-value"
                >{{ data.saas.currency }} {{ data.saas.arr.toLocaleString() }}</span
              ><span class="stat-label">{{ $t('superadmin.arr') }}</span>
            </div>
          </div>
          <div class="stat-card">
            <div class="stat-icon active"><i class="fas fa-credit-card"></i></div>
            <div>
              <span class="stat-value">{{ data.saas.paying_hotels }}</span
              ><span class="stat-label">{{ $t('superadmin.payingHotels') }}</span>
            </div>
          </div>
          <div class="stat-card">
            <div class="stat-icon signups"><i class="fas fa-user-plus"></i></div>
            <div>
              <span class="stat-value">{{ data.saas.new_signups_30d }}</span
              ><span class="stat-label">{{ $t('superadmin.newSignups30d') }}</span>
            </div>
          </div>
          <div class="stat-card">
            <div class="stat-icon trials"><i class="fas fa-stopwatch"></i></div>
            <div>
              <span class="stat-value">{{ data.saas.active_trials }}</span
          ...
          <div class="stat-card">
            ...
            <div>
              <span class="stat-value">{{ data.saas.conversion_rate }}%</span
        ...
        <div class="dash-grid">
          ...
          <div class="card dash-section">
            <h2>
              <i class="fas fa-hourglass-half"></i> {{ $t('superadmin.trialsExpiringTitle') }}
            </h2>
            <template v-if="data.saas.trials_expiring.length">
              <div
                v-for="tenant in data.saas.trials_expiring"
              ...
              >
                <div>
                  ...
                  <span class="muted"
                    >{{ $t('superadmin.trialEndsOn') }}
                    {{ new Date(tenant.trial_ends_at).toLocaleDateString() }}</span
                  >
                </div>
              </div>
            </template>
            <p v-else class="empty-mini">
              <i class="fas fa-circle-check"></i> {{ $t('superadmin.noTrialsExpiring') }}
            </p>
          </div>
        </div>
      </template>
      ...
      <div class="dash-grid">
        ...
        <div class="card dash-section">
          ...
          <div class="list-item">
            <div>
              <strong>{{ data.users_total }}</strong> {{ $t('superadmin.users') }}
            </div>
          </div>
          <div class="list-item">
            <div>
              <strong>{{ data.rooms_total }}</strong> {{ $t('superadmin.rooms') }}
            </div>
          </div>
          <div class="list-item">
            <div>
              <strong>{{ data.subscriptions.active_subscriptions }}</strong>
              {{ $t('superadmin.activeSubscriptions') }}
            </div>
          </div>
        </div>
        ...
        <div class="card dash-section">
          <div class="section-header-row">
            <h2>
              <i class="fas fa-building-circle-arrow-right"></i> {{ $t('superadmin.recentHotels') }}
            </h2>
            <router-link to="/superadmin/tenants" class="view-all-link"
              >{{ $t('superadmin.manageHotels') }} <i class="fas fa-arrow-right"></i
            ></router-link>
          </div>
          <div
            v-for="tenant in data.recent_registrations"
            :key="tenant.tenant_id"
            class="list-item"
          >
            <div>
          ...
          <p v-if="!data.recent_registrations?.length" class="empty-mini">
            <i class="fas fa-hotel"></i> {{ $t('superadmin.emptyHotels') }}
          </p>
        </div>
      </div>
    </template>
...
const maxGmv = computed(() =>
  Math.max(1, ...(data.value?.saas?.monthly_gmv?.map((month) => month.total) || [1])),
)
 ...
 * @param {number} number - The raw amount.
